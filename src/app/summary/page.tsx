...
export default function SummaryPage() {
  ...
  function buildShareText(): string {
    if (!data) return "";
    const lines = [
      "📋 Assistance Summary — ClaimIt",
      `📅 ${today}`,
      "",
    ];
    data.recommendations.forEach((r) => {
      lines.push(`• ${r.programName} — ${r.verdictLabel}`);
      if (r.steps[0]) lines.push(`  First step: ${r.steps[0]}`);
    });
    lines.push("", `🎯 Priority: ${data.priorityAction}`, "");
    lines.push("These recommendations are not an official decision. Final eligibility is determined by the relevant government authority.");
    return lines.join("\n");
  }
  ...
  async function handleShare() {
    ...
    if (navigator.share) {
      try { await navigator.share({ title: "ClaimIt Summary", text }); } catch { /* cancelled */ }
      return;
    }
    try {
  ...
  async function handleSaveImage() {
    ...
    try {
      ...
      link.download = `claimit-summary-${Date.now()}.png`;
    ...
    return (
      <div className="min-h-screen bg-[#fbf6ef] flex flex-col items-center justify-center px-6">
        <p className="text-[#6b6155] text-center mb-6 text-[15px]">
          Data not found. Start a new conversation.
        </p>
        <button
    ...
    <div className="min-h-screen bg-[#fbf6ef] flex flex-col">
      ...
      <header className="bg-[#fbf6ef] px-[22px] pt-3 pb-2 flex items-center justify-between">
        <div>
          ...
          <p className="text-[13px] text-[#a89c8c] mt-0.5">Created {today}</p>
        </div>
      ...
      <main className="flex-1 overflow-y-auto px-5 pt-1 pb-6">
        <div ref={summaryRef} className="bg-[#fbf6ef] pt-2 pb-4">
          ...
          <p className="text-[14px] text-[#4a4236] leading-[22.4px] mb-4">
            {data.recommendations.length} assistance programs found based on what you shared.
          </p>
          ...
          <div className="bg-[#f2ebe0] rounded-[14px] px-4 py-[15px]">
            <p className="text-[12px] text-[#8c8175] leading-[18px]">
              These recommendations are not an official decision. Final eligibility is determined by the relevant government authority.
            </p>
          </div>
        </div>
